...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_modules__WEBPACK_IMPORTED_MODULE_0__["default"])();
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = "Please, choose the size and material";
...
var drop = function drop() {
  ...
  function highlight(item) {
    item.closest('.file_upload').style.border = '3px solid #d645c8';
    item.closest('.file_upload').style.borderRadius = '15px';
    item.closest('.file_upload').style.transition = '0.1s';
...
var forms = function forms() {
  ...
  var clearInputs = function clearInputs() {
    ...
    upload.forEach(function (item) {
      item.previousElementSibling.textContent = 'Choose a file';
    });
  };
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      if (item.closest('.popup-design' || false)) {
        api = path.designer;
      } else {
        api = path.question;
      }
